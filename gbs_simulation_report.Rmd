...
```{r plots2.5 ,echo=FALSE}
# line plot comparing usable fragments in both digests 
prep_df %>%
	filter(num_ind < 600) %>%
	ggplot(aes(x = num_ind, y = mean_reads_per_fragment_per_ind, color = enzyme_name)) +
	geom_line(size = 2) +
	theme_bw() +
	theme(legend.justification = c(1, 1), 
				legend.position = c(1, 1),
				...
				legend.key = element_blank()) +
	xlab("Number of individuals") +
	ylab("Mean reads per fragment per individual")
```
...
prep_df %>%
	...
	facet_wrap(~enzyme_name)+
	xlab("Number of individuals")+
	ylab("Depth of top 1000 fragments (per individual)")
```
...
```{r plots4 ,echo=FALSE}
prep_df %>%
	filter(num_ind < 400, num_ind >= 100) %>%
	ggplot(aes(x = num_ind, y = mean_top_n_fragments, color = enzyme_name, ymin = min_top_n_fragments, ymax = max_top_n_fragments)) +
	geom_point(size = 2)+
	geom_errorbar()+
	theme_bw()+
